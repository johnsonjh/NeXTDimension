...
#ifdef NeXT
#if defined(I860)
#define N_PAGSIZ(x) 0x1000
#define N_SEGSIZ(x) 0x1000
#else
...
long Aflag = FALSE; /* print the Address during symbolic disassembly */
...
static void print_exec(struct exec *, struct shlib_info *);
static void print_mach(struct mach_header *);
static void print_loadcmds(struct mach_header *, struct load_command *);
...
static void get_text_info(struct mach_header *, struct load_command *,
			  long *, long *, long *);
static void get_data_info(struct mach_header *, struct load_command *,
			  long *, long *, long *);
static void get_trel_info(struct mach_header *, struct load_command *,
			  long *, long *, long *);
static void get_drel_info(struct mach_header *, struct load_command *,
			  long *, long *, long *);
...
static void get_gdb_info(struct mach_header *, struct load_command *,
			  long *, long *);
static void print_text(struct mach_header *,long, long, long, long, long , char *, char *);
static int print_label(long, long);
...
#if defined(I860)
static long i860_disassemble(char *, long);
static void i860_dump_operands( unsigned long, char *, char *, long );
static void i860_dump_addr( unsigned long, int, char *, long );
...
char *argv[];
{
	...
	if(!aflag && !hflag && !lflag && !Lflag && !tflag && !dflag && !oflag &&
	   ...
	   !Gflag && !cflag){
	    fprintf(stderr, "%s: One of -ahlLtdoOrSgc must be specified\n",
		    progname);
	    usage();
	    exit(1);
	}
	...
	for(i = 1; i < argc ; i++){
		...
		if(membername != NIL){
		    if(strncmp(membername, ar_hdr.ar_name,
		       sizeof(ar_hdr.ar_name)) == 0)
			process(fd, filename, ar_hdr.ar_name, &ar_hdr,
...
usage()
{
	...
	fprintf(stderr, "\t-g print the GDB symbol table\n");
...
long size;
{
	...
	if(membername != NIL && strcmp(membername, "__.SYMDEF") == 0){
	    if(Sflag)
	...
	if(vflag && hflag || lflag || Lflag || tflag || dflag || oflag ||
	   Oflag || rflag || gflag || Zflag || Gflag || cflag){
	    if(membername != NIL)
		printf("%s(%s):\n", filename, membername);
	    else
		printf("%s:\n", filename);
	...
	if(hflag || lflag || Lflag || tflag || dflag || oflag || Oflag ||
	   rflag || gflag || Zflag || Yflag || Bflag || Gflag || cflag){
	    ...
	    else if(exec.a_magic == LIBMAGIC){
		if(read(fd, (char *)&shlib, sizeof(shlib)) != sizeof(shlib)){
		    if(membername != NIL)
			fprintf(stderr, "%s : Can't read shared library header "
				"of %s(%s) (%s)\n", progname, filename,
	...
	if(rflag || Yflag || (tflag && Vflag)){
	    if(mach){
		get_trel_info(&mh, lcp, &trel_offset, &trel_size, &ntrels);
	    }
	    else{
	...
	if(rflag || Zflag){
	    if(mach){
		get_drel_info(&mh, lcp, &drel_offset, &drel_size, &ndrels);
	    }
	    else{
	...
	if(tflag && Vflag){
	    ...
	    qsort(sort_treloc, nsort_trelocs,
		  sizeof(struct relocation_info), rel_compare);
	}
	if(Bflag){
	    for(i = 0; i < nsyms; i++){
		if(syms[i].n_un.n_strx > 0 &&
	    ...
	    qsort(syms, nsyms, sizeof(struct nlist), symname_compare);
	}
	...
	if(hflag)
		...
		if(N_BADMAG(exec) && exec.a_magic != LIBMAGIC)
		    print_mach((struct mach_header *)&exec);
		else
		    print_exec(&exec, &shlib);
	    }

	if(mach && lflag && lcp != (struct load_command *)0)
	    print_loadcmds(&mh, lcp);
	...
	if(tflag){
	    if(mach){
		get_text_info(&mh, lcp, &text_offset, &text_size, &text_addr);
	    }
	    else{
	    ...
	    print_text(&mh, fd, offset, text_offset, text_size, text_addr,
		       filename, membername);
	}

	if(dflag || Gflag){
	    if(mach){
		get_data_info(&mh, lcp, &data_offset, &data_size, &data_addr);
	    }
	    else{
...
{
    return(rel1->r_address - rel2->r_address);
}
...
char *membername;
{
	...
	p[24] = '\0';
	if(vflag)
	    printf("%3d/%-3d%6d %s %s\n", uid, gid, size, p, par_hdr->ar_name);
	else
...
char *filename, *membername;
{
	...
	if(read(fd, &ran_size, sizeof(long)) != sizeof(long)){
	    fprintf(stderr,
		    "%s : Can't read number of ranlib structs of %s(%s) (%s)\n",
		    progname, filename, membername, sys_errlist[errno]);
	    return;
	}
...
struct shlib_info *shlib;
{
	...
	if(exec->a_magic == LIBMAGIC){
	    printf(" text_addr  data_addr  minor_version name\n");
	    printf(" 0x%08x 0x%08x %-13d %s\n", shlib->text_addr,
		   shlib->data_addr, shlib->minor_version, shlib->name);
	}
	else{
...
struct load_command *initlc;
{
	...
	for(i = 0 ; i < mh->ncmds; i++){
		...
		printf("load command extends past end of load commands\n");
	    switch(lc->cmd){
	    case LC_SEGMENT:
		...
		for(j = 0 ; j < sg->nsects ; j++){
			...
			if((s->flags & S_ZEROFILL) != 0)
			    printf(" S_ZEROFILL");
			if((s->flags & S_CSTRING_LITERALS) != 0)
			    printf(" S_CSTRING_LITERALS");
			if((s->flags & ~(S_ZEROFILL | S_CSTRING_LITERALS)) != 0
			   || s->flags == 0)
			    printf(" 0x%08x\n", s->flags);
			else
			    printf("\n");
		...
		printf("    cmdsize %d\n", ut->cmdsize);
#ifdef NeXT
	    	if(mh->cputype == CPU_TYPE_MC68030 &&
		   mh->cpusubtype == CPU_SUBTYPE_NeXT){
		   ...
		   mh->cpusubtype == CPU_SUBTYPE_BIG_ENDIAN){
		        struct NextDimension_thread_state_regs *NDcpu;
			flavor = *((unsigned long *)state);
			state += sizeof(unsigned long);
			count = *((unsigned long *)state);
			state += sizeof(unsigned long);
			NDcpu = (struct NextDimension_thread_state_regs *)state;
			if ( flavor == NextDimension_THREAD_STATE_REGS )
			    printf( "\tNextDimension code entry point 0x%08x\n",
			    		NDcpu->pc);
			else
			    printf( "\tMystery flavor %d (count %d)\n",
			    		flavor, count );
			break;
		}
#endif
...
/*
 * Get the infomation about the text section of a mach object file
 */
static
void
get_text_info(mh, lc, offset, size, addr)
struct mach_header *mh;
struct load_command *lc;
long *offset, *size, *addr;
{
    long i, j;
    struct segment_command *sg;
    struct section *s;
...
/*
 * Get the infomation about the data section of a mach object file
 */
static
void
get_data_info(mh, lc, offset, size, addr)
struct mach_header *mh;
struct load_command *lc;
long *offset, *size, *addr;
{
...
/*
 * Get the infomation about the text relocation of a mach object file
 */
static
void
get_trel_info(mh, lc, offset, size, num)
struct mach_header *mh;
struct load_command *lc;
...
/*
 * Get the infomation about the data relocation of a mach object file
 */
static
void
get_drel_info(mh, lc, offset, size, num)
struct mach_header *mh;
struct load_command *lc;
long *offset, *size, *num;
{
...
char *membername;
{
	...
	if(vflag){
		...
		if ( print_label(text_addr, 1) && Aflag )
			printf("%08x ", text_addr);
#if defined(I860)
	 	if(mh->cputype == CPU_TYPE_I860)
			j = i860_disassemble(p, text_addr);
		else
#endif
 ...
 * A 1 is returned if a label is printed, otherwise 0 is returned.
 */
static
int
...
long colon_and_newline;
{
	...
	while(high >= low){
		...
		{
		    if ( sort_syms[mid].n_type & N_EXT )
		    	printf( ":\t.globl %s\n", sort_syms[mid].n_un.n_name );
		    else
			printf(":\n");
		}
		return 1;
...
char *selectors;
{
	...
	for(i = 0; i < mlist->method_count; i++, method++){
	    ...
	    printf("\t\t       method_imp 0x%08x ",
		   method->method_imp);
	    if(vflag)
		(void) print_label((long)method->method_imp, 0);
	    printf("\n");
	}
}
...
long str_size;
{
	...
	for(i = 0 ; i < mhp->ncmds; i++){
		...
		for(j = 0 ; j < sg->nsects ; j++){
		    ...
		    printf("Relocation information (%s,%s) %d entries\n",
			   s->segname, s->sectname, s->nreloc);
		    lseek(fd, offset + s->reloff, 0);
		    if((read(fd, reloc, size)) != size){
			if(membername != NIL)
			    fprintf(stderr, "%s : Can't read relocation "
				    "entries for %s section of %s segment "
				    "from %s(%s) (%s)\n", progname, s->sectname,
				    s->segname, filename, membername,
				    sys_errlist[errno]);
			else
			    fprintf(stderr, "%s : Can't read relocation "
				    "entries for %s section of %s segment "
				    "from %s (%s)\n", progname, s->sectname,
				    s->segname, filename, sys_errlist[errno]);
		    }
		    else{
			if(s->nreloc != 0)
...
#if defined(I860)
	if(mhp->cputype == CPU_TYPE_I860)
		printf("address  pcrel length reloc_type    extern symbolnum\n");
	else
#endif
		printf("address  pcrel length extern symbolnum\n");
	for(i = 0 ; i < nreloc ; i++){
	    p = reloc + i;
	    if(vflag){
	    	...
	    	if(mhp->cputype == CPU_TYPE_I860)
		{
			...
			{
				case RELOC_VANILLA:
					printf( "%-14s","RELOC_VANILLA");
					break;
					
				case RELOC_PAIR:
					printf( "%-14s","RELOC_PAIR");
					break;
					
				case RELOC_HIGH:
					printf( "%-14s","RELOC_HIGH");
					break;
					
				case RELOC_LOW0:
					printf( "%-14s","RELOC_LOW0");
					break;
					
				case RELOC_LOW1:
					printf( "%-14s","RELOC_LOW1");
					break;
					
				case RELOC_LOW2:
					printf( "%-14s","RELOC_LOW2");
					break;
					
				case RELOC_LOW3:
					printf( "%-14s","RELOC_LOW3");
					break;
					
				case RELOC_LOW4:
					printf( "%-14s","RELOC_LOW4");
					break;
					
				case RELOC_SPLIT0:
					printf( "%-14s","RELOC_SPLIT0");
					break;
					
				case RELOC_SPLIT1:
					printf( "%-14s","RELOC_SPLIT1");
					break;
					
				case RELOC_SPLIT2:
					printf( "%-14s","RELOC_SPLIT2");
					break;
					
				case RELOC_HIGHADJ:
					printf( "%-14s","RELOC_HIGHADJ");
					break;
					
				case RELOC_BRADDR:
					printf( "%-14s","RELOC_BRADDR");
					break;
		...
		if(p->r_extern){
	    	...
	    	if(mhp->cputype == CPU_TYPE_I860)
		{
		    printf("%08x %1d     %-2d     %-2d            %1d      %d\n",
			p->r_address, p->r_pcrel, p->r_length, p->r_type,
			 p->r_extern, p->r_symbolnum );
		}
		else
#endif
		{
		    printf("%08x %1d     %-2d     %1d      %d\n",
			p->r_address, p->r_pcrel, p->r_length,
			p->r_extern, p->r_symbolnum);
		}
	    }
	}
...
long addr;
{
	...
	case 0x0:
		...
		case 1:
		    ...
		    break;
		case 2:
		    printf("cmpl\t#0x%x,", *(long *)(text + length));
		    length += sizeof(long);
		    length += print_ef(MODE(opword), REG(opword),
				       text + length, L_SIZE);
		    break;
...
static
long
i860_disassemble(text, addr)
char *text;
long addr;
{
	unsigned char *ins_bytes = (unsigned char *) text;
	...
	opcode = ins_bytes[0]|(ins_bytes[1]<<8)|(ins_bytes[2]<<16)|(ins_bytes[3]<<24);
#else
	/* Convert from big-endian format */
	opcode = ins_bytes[3]|(ins_bytes[2]<<8)|(ins_bytes[1]<<16)|(ins_bytes[0]<<24);
#endif
	if ( opcode == 0 )
	{	/* The pad opcode, 0, is chosen as an illegal insn to fault if executed*/
		printf( "|	Padded to i860 segment bound.\n" );
		return( 4 );
	}
	/*
	 ...
	 */
	for( op = (struct i860_opcode *)i860_opcodes, i = 0; i < NUMOPCODES; ++i,++op )
	{
		if ( (opcode & op->mask) == op->match )	/* We got one! */
		{
			if ( isdual )
				printf( "\td.%-12s\t", op->name );
			else
				printf( "\t%-12s\t", op->name );
			i860_dump_operands( opcode, (char *)op->args, text, addr );
			return 4;	/* All i860 instructions are 4 bytes long */
		}
	}
	/* Didn't find the opcode.  Dump it as a .long directive. */
	/* Build it as a little-endian insn, in a format to match asm */
	opcode = ins_bytes[3]|(ins_bytes[2]<<8)|(ins_bytes[1]<<16)|(ins_bytes[0]<<24);
	printf( "\t.long\t0x%08x\t| Unknown instruction\n", opcode );
...
static char *i860_controlregs[] = { "fir", "psr", "dirbase", "db", "fsr", "epsr",
 ...
 static void
i860_dump_operands( opcode, format, text, addr )
    unsigned long opcode;
    char *format;
    char *text;
    long addr;
	...
	while ( *format != '\0' )
	{
		switch( *format )
		{
			...
			case 'D': /* constant for shift opcode */	
				printf( "%d", opcode & 0xFFFF );
				break;
				
   			case 'B': /*	5 bit immediate, for bte and btne insn */
				printf( "%d", GET_RS1(opcode) );
				break;
...
{
	long high, low, mid;
	struct relocation_info *rp = (struct relocation_info *) NULL;
	struct relocation_info *pairp = (struct relocation_info *) NULL;
	...
	if ( nsort_trelocs )
	{
	    ...
	    mid = (high - low) / 2;
	    while(high >= low){
		if(sort_treloc[mid].r_address == addr){
		    rp = &sort_treloc[mid];
		    break;
		}
		if(sort_treloc[mid].r_address > addr){
	...
	if ( rp != NULL && rp->r_type == RELOC_PAIR )
	{
		pairp = rp;
		if ( rp > sort_treloc && (rp-1)->r_address == rp->r_address )	
			--rp;
		else if ( rp < &sort_treloc[nsort_trelocs-1] && 
			 (rp+1)->r_address == rp->r_address )
		{
			++rp;
		}
		else
		{
			fprintf(stderr,"Stray RELOC_PAIR: 0x%08x\n",pairp->r_address);
			rp = (struct relocation_info *) NULL;
			pairp = (struct relocation_info *) NULL;
		}
	}
	if ( rp != NULL && (rp->r_type == RELOC_HIGH || rp->r_type == RELOC_HIGHADJ) )
	{
		pairp = rp;
		if ( rp > sort_treloc && (rp-1)->r_address == rp->r_address )	
			--pairp;
		...
		{
			fprintf(stderr,"No RELOC_PAIR: 0x%08x\n",pairp->r_address);
			rp = (struct relocation_info *) NULL;
			pairp = (struct relocation_info *) NULL;
		}
	}
	/* Guess a prefix code for the immediate value */
	if ( (rp != NULL && rp->r_type == RELOC_HIGH) || format == 'I' )
		prefix = "h%";
	else if ( (rp != NULL && rp->r_type == RELOC_HIGHADJ) || format == 'J' )
		prefix = "ha%";
	else if (rp != NULL && rp->r_type >= RELOC_LOW0 && rp->r_type <= RELOC_SPLIT2)
	{
	...
	if ( rp != NULL && (rp->r_type == RELOC_HIGH || rp->r_type == RELOC_HIGHADJ) )
	{
		if ( pairp->r_type == RELOC_PAIR )
			addr_field = (addr_field << 16)|(pairp->r_symbolnum & 0xFFFF);
	}	
	if ( prefix != NULL )
		printf( "%s", prefix );
	...
	{
	    if ( print_symbol( addr + 4 + ((long)addr_field), addr ) == TRUE )
		    return;
	    printf(".%+d", addr_field + 4);
	    return;
	}
	if ( print_symbol( addr_field, addr ) == TRUE )	
		return;
		
	/* we can't find anything else to do with it. */
	printf( "0x%x", addr_field );
	return;
}
#endif
