 ...
 * use the equivalence between FRAME and struct frame_info *.
 */
...
struct frame_info
  {
    ...
    /* Pointers to the next and previous frame_info's in this stack
       Note that these pointers will be active even though this thing
       will be in an array.  This means that stack depth and next and
       previous stack frames need have no relation to neighbors in the
       cache.  This allow creation of arbitrary frame in the cache
       without interfering with the caller-callee relation. */
    FRAME next, prev;
  };
