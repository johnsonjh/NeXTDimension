...
#ifdef	MACH_EXC
/* Exception port connected to our debugged inferior. */
port_t inferior_exception_port;

/* What used to be our inferior's exception port */
port_t inferior_old_exception_port;

/* are we stopped by mach or ptrace? */
boolean_t stopped_in_ptrace;

/* did WE stop all of the other threads before we did the task_resume? */
     ...
     char *addr_exp;
{
  ...
  for (i = 0; i < NUM_REGS; i++)
      ...
      if (addr_exp == 0 && i > 0 && i % 16 == 0)
	{
	  ...
	  /* Quit if the user types 'q' or 'Q' */
	  if (index(l, 'q') || index(l, 'Q'))
	    {
	      dont_repeat();
	      break;
	    }
