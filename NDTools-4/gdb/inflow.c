...
#ifdef SYSV
static struct termio inferior;
static struct termio ours;
#else /* !SYSV */
     ...
     char **env;
{
  ...
  pid = vfork ();
  if (pid < 0) perror_with_name ("vfork");
...
/* Not needed on Sun, at least, and loses there
   because it clobbers the superior.  ?????????? */
      ...
      call_ptrace (0); /* trace child process */
                       /* other args in ptrace call are ignored */
...
#ifdef MACH
#ifdef	MACH_EXC  /* mach exceptions */
      ret = port_allocate(task_self(), &inferior_exception_port);
      if (ret != KERN_SUCCESS) {
	fprintf(stderr, "create_inferior: ");
	mach_error("port_allocate", ret);
	exit(1);
      }
/*  MACH ports are created restricted and we want to use it that way */
/*  i.e. receiver must name port explicitly and cannot use PORT_ENABLED */
/*
      ret = port_unrestrict(task_self(), inferior_exception_port);
      if (ret != KERN_SUCCESS) {
	fprintf(stderr, "create_inferior: ");
	mach_error("port_unrestrict", ret);
	exit(1);
      }
*/
#endif	MACH_EXC
      ...
      if (ret != KERN_SUCCESS) {
	fprintf(stderr, "create_inferior: ");
	mach_error("task_by_unix_pid", ret);
	exit(1);
      }
	...
	exit (1);
      ...
      ret = vm_deallocate(task_self(), thread_list,
	...
	mach_error("vm_deallocate", ret);
      ...
      if (ret != KERN_SUCCESS) {
	fprintf(stderr, "create_inferior: ");
	mach_error("task_set_special_port", ret);
	exit(1);
      }
#endif	MACH_EXC
#endif	MACH
...
kill_command ()
{
  ...
  if (!query ("Kill the inferior process? ")) error ("Not confirmed.");
  kill_inferior ();
}
...
inferior_died ()
{
  inferior_pid = 0;  attach_flag = 0;
#ifdef MACH
  inferior_task = PORT_NULL;
#endif MACH
  mark_breakpoints_out ();  reopen_exec_file ();
  if (have_core_file_p ())
