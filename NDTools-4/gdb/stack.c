     ...
     int args;
{
  ...
  if (func) funname = SYMBOL_NAME (func);
  else {
      register int misc_index = find_pc_misc_function (fi->pc);
      if (misc_index >= 0) funname = misc_function_vector[misc_index].name;
  }

  if (source >= 0 || !sal.symtab) {
      if (level >= 0) printf ("#%-2d ", level);
      if (fi->pc != sal.pc || !sal.symtab) printf ("0x%x in ", fi->pc);
  ...
  if (source != 0 && sal.symtab)
    {
      ...
      if (!done)
	{
	  if (mid_statement) printf ("0x%x\t", fi->pc);
     ...
     char *count_exp;
{
  ...
  if (count_exp)  count = parse_and_eval_address (count_exp);
  else  count = -1;
  for (i = 0, frame = get_current_frame ();
       frame && count--;
