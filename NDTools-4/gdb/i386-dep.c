...
kill_inferior ()
{
  ...
  if (remote_debugging || inferior_pid == 0) return;
#ifdef	MACH_EXC
  if (! stopped_in_ptrace) {
    kill (inferior_pid, SIGHUP);
    if ((ret = task_resume(inferior_task)) != KERN_SUCCESS) {
      fprintf( stderr, "kill_inferior: "); mach_error("task_resume", ret);
      exit (1);
    }
    wait (&w);
    if (WIFEXITED (w)) { inferior_died (); return;}
  }
#endif	MACH_EXC
  ptrace (8, inferior_pid, 0, 0); wait (0); inferior_died ();
}
     ...
     int signal;
{
  ...
  if (!stopped_in_ptrace) {
    if (step) {
      suspend_all_threads ();  set_trace_bit (current_thread);
      if ((ret = thread_resume (current_thread)) != KERN_SUCCESS) {
	fprintf (stderr, "resume: "); mach_error ("thread_resume", ret);
	exit (1);
      }
      other_threads_suspended = TRUE;
    } else other_threads_suspended = FALSE;
    if ((ret = task_resume (inferior_task)) != KERN_SUCCESS) {
      fprintf(stderr, "resume: "); mach_error("task_resume", ret);
      exit (1);
    }
  } else { /* stopped in ptrace */
    if (remote_debugging) remote_resume (step, signal);
    else {
...
fetch_inferior_registers ()
{
  ...
  if ( !inferior_pid) return;
  /* see <machine/thread_status.h> */
     ...
     int regno;
{
  ...
  } else load_register( regno, read_register( regno), state);
     ...
     int len;
{
  ...
  static vm_address_t last_vm_addr = 0;
  static vm_size_t last_vm_size = 0;
  static boolean_t vm_cache_in_use = FALSE;

  /* compute page aligned numbers */
  vm_addr = memaddr - (memaddr % vm_page_size);
  vm_size = len + (vm_page_size - (len % vm_page_size));

  /* Do we really need to do a vm_read? */
  if ( !vm_read_cache_valid || !vm_cache_in_use ||
       (vm_addr != last_vm_addr) || (vm_size != last_vm_size)) {
    /* 
     * deallocate the old cache if it is being used
     */
    if (vm_cache_in_use) {
      ret = vm_deallocate(task_self(), (vm_address_t) buff,
			  (vm_size_t) buff_size);
      if (ret != KERN_SUCCESS) {
	fprintf(stderr, "read_inferior_memory: ");
    ...
    if (ret != KERN_SUCCESS) {
#ifdef	DEBUG
      fprintf(stderr, "read_inferior_memory(0x%x, 0x%x, %d)\n", memaddr,
	      myaddr, len);
      fprintf(stderr, "vm_read(0x%x, 0x%x, 0x%x, 0x%x, %d)\n", inferior_task,
	      vm_addr, vm_size, &buff, buff_size);
#endif	DEBUG
      mach_error ("vm_read", ret);
      vm_read_cache_valid = FALSE;  vm_cache_in_use = FALSE;
      *myaddr = 0;
      return (1);
    }
  }
  /* copy the relavent bytes to our caller */
  bcopy((char *)(buff + (memaddr % vm_page_size)), myaddr, len);
  last_vm_addr = vm_addr;  last_vm_size = vm_size;
  vm_cache_in_use = TRUE;  vm_read_cache_valid = TRUE;
  return( 0);
     ...
     int len;
{
  ...
  ret = vm_read (inferior_task, vm_addr, vm_size, &buff, &buff_size);
  if (ret != KERN_SUCCESS) {
    fprintf (stderr, "Error in write_inferior_memory()\n");
    mach_error ("vm_read", ret);
    exit (1);
  }
   ...
   *
   * If we get a protection failure, then up the protection so that
   * we CAN write, do the write, and put it back.  Since we can't
   * find out what the protection is, we guess at read+execute.
   */
  
  ret = vm_write (inferior_task, vm_addr, buff, vm_size);
  if (ret == KERN_PROTECTION_FAILURE) {
    /* do a vm_region() to get the protection */
    prot = VM_PROT_WRITE | VM_PROT_READ | VM_PROT_EXECUTE;
    ret = vm_protect (inferior_task, vm_addr, vm_size, FALSE, prot);
    if (ret != KERN_SUCCESS) {
      fprintf (stderr, "Error in write_inferior_memory()\n");
      mach_error ("vm_protect", ret);
      exit (1);
    }
    ret = vm_write (inferior_task, vm_addr, buff, vm_size);
    if (ret != KERN_SUCCESS) {
      fprintf (stderr, "Error in write_inferior_memory()\n");
      mach_error ("vm_write", ret);
      exit (1);
    }
    prot &= ~VM_PROT_WRITE;
    ret = vm_protect (inferior_task, vm_addr, vm_size, FALSE, prot);
  ...
  /* deallocate that page we needed.. */
  ret = vm_deallocate(task_self(), (vm_address_t) buff, (vm_size_t) buff_size);
  if (ret != KERN_SUCCESS) {
    fprintf (stderr, "Error in write_inferior_memory()\n");
    mach_error ("vm_deallocate", ret);
    exit (1);
  ...
  if (remote_debugging) buffer[0] = remote_fetch_word (addr);
  else buffer[0] = ptrace (1, inferior_pid, addr, 0);
  ...
  for (i = 0; i < count; i++, addr += sizeof (int))
    {
      errno = 0;
      if (remote_debugging) remote_store_word (addr, buffer[i]);
      else ptrace (4, inferior_pid, addr, buffer[i]);
...
static load_register( regno, value, state)
int regno, value;
...
{ 
  switch ( regmap[regno]) { 
    case EAX: state->eax = value; break;
    case ECX: state->ecx = value; break;
    case EDX: state->edx = value; break;
    case EBX: state->ebx = value; break;
    case UESP: state->uesp = value; break;
    case EBP: state->ebp = value; break;
    case ESI: state->esi = value; break;
    case EDI: state->edi = value; break;
    case EIP: state->eip = value; break;
    case EFL: state->efl = value; break;
    case CS: state->cs = value; break;
    case SS: state->ss = value; break;
    case DS: state->ds = value; break;
    case ES: state->es = value; break;
    case FS: state->fs = value; break;
    case GS: state->gs = value; break;
    default: 
...
static store_registers( state, regs)
...
#ifdef ORC
  fprintf( stderr, "i386_float_info: No floating pt. support in Mach yet!\n");
  return;
#else ORC
