...
end_symtab ()
{
  ...
  if (pending_blocks == 0 && file_symbols == 0 && global_symbols == 0)
    {
      free (line_vector);
      line_vector = 0; last_source_file = 0; line_vector_length = -1;
      return;
    }
...
unrecord_misc_function ()
{
	if (misc_bunch_index == 0)
		error ("Internal error processing symbol table.");
	misc_bunch_index--;
	misc_count--;
}
...
static dump_sym_tab()
{  register struct symtab *st = symtab_list;

   fprintf( stderr, "dumping sym tab list \n");
   while ( st != NULL) {
     fprintf( stderr, "fn = %s\n", st->filename);
     st = st->next;
   }
...
#ifdef LINENO_BUG
		/* We must validate the lnnoptr to handle ld860 bug
		 */
		{
		  long validate_line_ptr();

		  fcn_line_ptr = validate_line_ptr(fcn_line_ptr,
				fcn_cs_saved.c_symnum);
		}
		enter_linenos (fcn_line_ptr, fcn_first_line, fcn_last_line);

		new = context_stack;

		/* Some compilers (eg Metaware) forget the function length;
		 * if that happens, get a reasonable value from .ef
		 */
		{
		  unsigned long end;

		  if (fcn_aux_saved.x_sym.x_misc.x_fsize)
	 	    end = fcn_cs_saved.c_value +
...
#endif /* AOUT_MAGIC */
#ifdef AOUTHDR_VARIABLE
	aouthdr_size = file_hdr->f_opthdr;
#endif
    ...
    int size;
{
  ...
  if (size < sizeof (AOUTHDR))
    return -1;
  if (myread (chan, (char *)aout_hdr, sizeof(AOUTHDR)) != sizeof(AOUTHDR))
    return -1;
#else
  if (size != sizeof (AOUTHDR))
    return -1;
...
#ifdef AOUTHDR_VARIABLE
  if (lseek (chan, FILHSZ + aouthdr_size, 0) < 0)
#else
...
{
  cs->c_symnum = symnum;
  if (fread ((char *)sym, SYMESZ, 1, nlist_stream_global) != 1)
    error("cannot read symbol table entry at symnum=%d\n", symnum);

  cs->c_nsyms = (sym->n_numaux & 0xff) + 1;
  if (cs->c_nsyms == 2)
    {
      /* doc for coff says there is either no aux entry or just one */
      if (fread ((char *)aux, AUXESZ, 1, nlist_stream_global) != 1)
	error ("cannot read aux symbol table entry at symnum=%d\n", symnum);
    }
...
#ifdef LINENO_BUG
/* Fix up the ld860 line-number bug
 */
long
validate_line_ptr(file_offset, symnum)
    long file_offset;
    int symnum;
{
  ...
  register int lno;
  static int hint_lno = 0;	/* try to avoid full table search */
  int begin_lno, end_lno;
  int try;
  static int first = 1;

  /* See if file pointer into lineno table is valid */
  if (file_offset >= linetab_offset &&
      file_offset < linetab_offset + linetab_count * LINESZ &&
      ((file_offset - linetab_offset) % LINESZ) == 0)
    {
      rawptr = &linetab[file_offset - linetab_offset];
      bcopy(rawptr, &lineno, LINESZ);
      if (lineno.l_lnno != 0 || lineno.l_addr.l_symndx != symnum)
	return(file_offset);
    }
  ...
  if (first)
    {
      first = 0;
      printf("\nWARNING: Line number information is invalid.  Rebuilding... ");
      fflush (stdout);
    }

  /* Avoid calculation of invalid pointer */
  if (hint_lno >= linetab_count)
    hint_lno = 0;
  ...
  for (try = 0; try < 2; try++)
    {
      if (try == 0)
	{
	  /* First try - from hint to end of table */
	  begin_lno = hint_lno;
	  end_lno = linetab_count;
	}
      else
	{
	  /* Second try - from beginning to hint */
	  begin_lno = 0;
	  end_lno = hint_lno;
	}

      rawptr = linetab + begin_lno * LINESZ;
      for (lno = begin_lno; lno < end_lno; lno++)
	{
	  bcopy(rawptr, &lineno, LINESZ);
	  if (lineno.l_lnno == 0 && lineno.l_addr.l_symndx == symnum)
	    {
	      file_offset = (int) (rawptr - linetab) + linetab_offset;
	      /*
	      printf("\nlinenums for symbol %d at offset 0x%x\n", symnum, 
		    file_offset);
	      */
	      hint_lno = lno + 1;	/* start next search from here */
	      return(file_offset);
	    }
	  rawptr += LINESZ;
	}
...
/* Fixed to avoid alignment violations - Jim Hanko
 */
static void
    ...
    register int last_line;
{
  ...
  for (bcopy(rawptr, &lineno, LINESZ);
	lineno.l_lnno && lineno.l_lnno <= last_line;
	rawptr += LINESZ, bcopy(rawptr, &lineno, LINESZ))
    record_line (first_line + lineno.l_lnno, lineno.l_addr.l_paddr);
}

static int
hashname (name)
...
#ifdef ORC
          /* this fixes a bug that prevents functions like
               foo( x)
               register struct bar *x;
             from being stack traced properly in printcmd.c:print_frame_args
	     CB, ORC, June 89
	  */
	  case C_REGPARM:
...
#ifdef STRUCT_SYMBOL_BUG
  if (cs->c_nsyms > 1 && aux->x_sym.x_tagndx != 0 && 
	aux->x_sym.x_fcnary.x_fcn.x_endndx == 0)
#else
  if (cs->c_nsyms > 1 && aux->x_sym.x_tagndx != 0)
#endif
    {
...
#ifdef T_ARG_BUG
	/* but it does ... so we'll convert it to void */
...
#ifdef STRUCT_SYMBOL_BUG
  while (symnum < nlist_nsyms_global)
#else
