     ...
     int len;
{
  ...
  while (len > 0)
...
#ifdef NONSTANDARD_MEMORY_LAYOUT
	...
	{
	  if (memaddr < data_start)
	    {
	      i = min (len, data_start - memaddr);
	    }
	  else if (memaddr >= data_end && memaddr < text_start)
	    {
...
_initialize_core()
{
  ...
  exec_data_start = 0;
  exec_data_end = 0;
  /* stack_start & stack_end are REALLY initialized in dep.c: core_file_command */
