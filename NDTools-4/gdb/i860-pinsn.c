     ...
     FILE *stream;		/* stream on which to write result */
{
  ...
  switch (insn.gen.op1)
    ...
    case 001:
      ...
      fprintf (stream, "ld.b %d(%s),%s", offset, 
    ...
    case 002:
      ...
      break;
    case 003:
      fprintf (stream, "st.b %s,%d(%s)", ireg[insn.gen.src1], split_offset,
		ireg[insn.geni.src2]);
      break;
    case 004:
    ...
    case 005:
      offset = SIGN_EXT(16, insn.geni.offset);
      fprintf (stream, "ld.%c %d(%s),%s", (insn.geni.offset & 1) ? 'l' : 's',
		(offset & ~1), ireg[insn.geni.src2], ireg[insn.geni.dest]);
      break;

    case 007:
      fprintf (stream, "st.%c %s,%d(%s)", (insn.geni.offset & 1) ? 'l' : 's',
    ...
    case 011:
      ...
      fprintf (stream, "fld.%c %d(%s)%s,f%d", suffix,
    ...
    case 013:
      ...
      fprintf (stream, "fst.%c f%d,%d(%s)%s", suffix,
    ...
    case 015:
      ...
      fprintf (stream, "flush %d(%s)%s", offset, ireg[insn.gen.src2], 
		ai ? "++" : "");
      break;
    case 016:
    ...
    case 017:
      ...
      fprintf (stream, "pst.d f%d,%d(%s)%s", insn.gen.dest, 
    ...
    case 024:
      ...
      break;
    case 025:
      fprintf (stream, "btne %d,%s,", insn.gen.src1, ireg[insn.gen.src2]);
      offset = split_offset << 2;
      print_address ((CORE_ADDR) (memaddr + 4 + offset), stream);
      break;
    case 026:
      ...
      break;
    case 027:
      fprintf (stream, "bte %d,%s,", insn.gen.src1, ireg[insn.gen.src2]);
      offset = split_offset << 2;
      print_address ((CORE_ADDR) (memaddr + 4 + offset), stream);
      break;
    ...
    case 031:
      ...
      fprintf (stream, "pfld.%c %d(%s)%s,f%d", suffix, 
      		offset, ireg[insn.gen.src2], ai ? "++" : "", insn.gen.dest);
      break;
    case 032:
...
gen_irr(name, insn, immed, stream)
    ...
    FILE *stream;
{
  fprintf (stream, "%s %d,%s,%s", name, immed,
	    ireg[insn.gen.src2], ireg[insn.gen.dest]);
}
