     ...
     int signal;
{
  ...
  if (!one_stepped)
      ...
      if (br == bicca && pc8 != next_pc)
	{
	  ...
	  write_memory (pc8, break_insn, break_insn);
	}
...
/*
 * Find the pc saved in frame FRAME.  Note that this is an address,
 * not a frame id.
 */
CORE_ADDR
     ...
     FRAME_ADDR frame;
{
  ...
  /* Sigh.  The *real* answer to this problem is to make
     FRAME_SAVED_PC work with FRAMEs instead of FRAME_ADDRs,
     but until then ...  */

  /* If it's at the bottom, the return value's stored in i7/rp */
  if (FRAME_FP (get_current_frame ()) == frame)
    prev_pc = GET_RWINDOW_REG (read_register (SP_REGNUM), rw_in[7]);
  else
    {
      for (fid = get_prev_frame (get_current_frame ());
	   fid && FRAME_FP (fid) != frame;
	   fid = get_prev_frame (fid))
	;

      if (!fid)
	fatal ("frame_saved_pc called with an invalid frame\n");

      fci = get_frame_info (fid);

      if (!(fci->bottom))
	fatal ("frame_saved_pc called with an invalid frame\n");

      prev_pc = GET_RWINDOW_REG (fci->bottom, rw_in[7]);
    }
...
/* Set *target if we find a branch.  This is in pinsn.c instead of
   dep.c because it uses the insn_fmt union.  */
