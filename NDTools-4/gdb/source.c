     ...
     char **filename_opened;
{
  ...
  for (p = path; p; p = p1 ? p1 + 1 : 0)
    {
      ...
      strncpy (filename, p, len);    filename[len] = 0;
      strcat (filename, "/");  strcat (filename, string);
  ...
  if (filename_opened)
    if (fd < 0) *filename_opened = (char *) 0;
    else if (filename[0] == '/')
      *filename_opened = savestring (filename, strlen (filename));
    else *filename_opened = concat (current_directory, "/", filename);
     ...
     int noerror;
{
  ...
  if (desc < 0)
    {
      extern int errno;
      if (! noerror) perror_with_name (s->filename);
