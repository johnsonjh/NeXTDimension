...
#ifdef ROBOTUSSIN
#include "robotussin.h"
#define AOUTHDR		union all_headers
#else /* !ROBOTUSSIN */
#ifndef COFF_FORMAT
...
/* **** Some indication that there should be some modifications
   for ROBOTUSSIN here */
...
#if 0
      /* This should no longer be needed, as lookup_block_symbol
	 checks specifically to make sure that it gets a register or
	 other declaration before it gets a parameter declaration.
	 This is only done for the unsorted blocks, so compare_symbols
	 still must take register vs. non-register symbols into account.
	 */
      else
	{
...
struct symbol_existence {
  char *name;
  int next;			/* Index into symbol_existence_buffer */
};

/*
 * Buffer for allocating space for symbol existence entries on the
 * tmp_symchain.  Needs to be global since process_symbol_for_psymtab
 * uses it.
 *
 * I'm starting to get tempted to merge that subroutine in.
 */
static struct symbol_existence *symbol_existence_buffer;
static struct symbol_existence *existence_alloc_point;
static int symbol_existence_buffer_size;

static void
free_symbol_existence_buffer()
{
  free (symbol_existence_buffer);
}
     ...
     int text_size;
{
  ...
  /* For keeping track of global symbols to decide who is a misc
     function */
  int tmp_symchain[HASHSIZE];	/* Index into symbol_existence_buffer */
  int *prevtmp;
  struct symbol_existence *curtmp;
  int found;
...
#define HASH_OFFSET 0
  ...
  symbol_existence_buffer_size = nlistlen / 15;
  symbol_existence_buffer = (struct symbol_existence *)
    xmalloc (symbol_existence_buffer_size * sizeof (struct symbol_existence));
  existence_alloc_point = symbol_existence_buffer;

  old_chain = make_cleanup (free_symbol_existence_buffer, 0);
  ...
  /* Zero out chain of misc functions */
  {
    int i;
  ...
  make_cleanup (free_all_psymtabs, 0);
  ...
  for (symnum = 0; symnum < nlistlen; symnum++)
	...
	case N_BSS | N_EXT:
	  ...
	  /* We need to check to see if this symbol is the same as one
	     of the debugger symbols we already have.  If it is, we
	     don't worry about it (read_ofile_symbols will).  If it
	     isn't, we have to record it in record_misc_bunches */
	  
	  found = 0;
	  nsl = strlen (namestring + HASH_OFFSET);
	  if (nsl > 10) nsl = 10; /* Keep from going off the end of */
				  /* the earth below */
	  hash = hashname (namestring + HASH_OFFSET);
	  prevtmp = &(tmp_symchain[hash]);
	  curtmp = symbol_existence_buffer + *prevtmp;
	  while (curtmp >= symbol_existence_buffer)
	    {
	      /* The only case in which the following would be a bad
		 idea is if the string the curtmp->name points at is
	         near the end of the segment in which it finds itself
	         pointing (in practice the stack; see
	         symbol_file_command and others), and the distance
	         from it to the end of the segment is shorter than 10
		 chaacters.  Given that stringtab was allocated in the
		 parent routine to this one on the stack and that this
		 routine has *alot* of local variables, I don't think
		 that this will ever be a problem. */
	      if (
#ifdef NAMES_HAVE_UNDERSCORE
		  *namestring == '_' &&
#endif
		  namestring[HASH_OFFSET + nsl-1] == curtmp->name[nsl-1]
		  && namestring[HASH_OFFSET + nsl-2] == curtmp->name[nsl-2]
#ifdef PROFILE_TYPES
		  && (strcmp_called++, 1)
#endif
		  && !strcmp (namestring + HASH_OFFSET,
			      curtmp->name))
		{
		  found = 1;
		  /* Remove from list */
...
#ifdef PROFILE_TYPES
  {
    int i;
    for (i = 0; i< 256; i++)
      fprintf (stderr, "type: 0x%x -- hits: %d\n", i, profile_type[i]);
    fprintf (stderr, "strcmp called (for external syms): %d times\n",
	     strcmp_called);
  }
#endif
}
 ...
 */
static void
process_symbol_for_psymtab (name, chain)
     char *name;
     int chain[];
{
  ...
  if (misc_chain_correspond)
    {
      /* Need to put on chain.  Note that debugger symbols do *not*
	 have _'s in them. */

      hash = hashname (SYMBOL_NAME (sym));

      /* Get some space from the buffer */
      while (existence_alloc_point >=
	     symbol_existence_buffer + symbol_existence_buffer_size)
	{
	  int diff = existence_alloc_point - symbol_existence_buffer;
	  
	  symbol_existence_buffer = (struct symbol_existence *)
	    xrealloc (symbol_existence_buffer,
		      (symbol_existence_buffer_size *= 2) *
		      sizeof (struct symbol_existence));
	  existence_alloc_point = symbol_existence_buffer + diff;
#ifdef DEBUGINFO
	  fprintf(stderr, "debuginfo: Had to realloc symbol existence buffer\n");
#endif
	}
      curtmp = existence_alloc_point++;
      curtmp->name = SYMBOL_NAME (sym);
      curtmp->next = chain[hash];
      chain[hash] = curtmp - symbol_existence_buffer;
    }
}
#undef HASH_OFFSET
