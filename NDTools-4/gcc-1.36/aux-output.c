/* Subroutines for insn-output.c for i860.
     ...
     rtx *operands;
{
  ...
  if (CONSTANT_P (operands[3]))
    {
      if (GET_CODE (operands[3]) == CONST_INT && LOGIC_INT (operands[3]) )
      {
	output_asm_insn ("or %3,r0,%0 /*aux-ldaddr*/", operands);
	return;
      }
      if (GET_CODE (operands[3]) == CONST_INT && SMALL_INT (operands[3]) )
      {
	output_asm_insn ("adds %3,r0,%0 /*aux-ldaddr*/", operands);
	return;
      }
      output_asm_insn ("orh h%%%3,r0,%0\n\tor l%%%3,%0,%0 /*aux-ldaddr*/",operands);
...
/*  output_asm_insn ("mov %1,%0", xoperands); */
  if (GET_CODE (xoperands[1]) == REG)
    output_asm_insn ("mov %1,%0 /*sz_blk_mv*/", xoperands);
  else if (GET_CODE (xoperands[1]) == CONST_INT && LOGIC_INT (xoperands[1]) )
    output_asm_insn ("or %1,r0,%0 /*sz_blk_mv*/", xoperands);
  else if (GET_CODE (xoperands[1]) == CONST_INT && SMALL_INT (xoperands[1]) )
    output_asm_insn ("adds %1,r0,%0 /*sz_blk_mv*/", xoperands);
  else
    output_asm_insn ("orh h%%%1,r0,%0\n\tor l%%%1,%0,%0 /*sz_blk_mv*/", xoperands );
#else
     ...
     rtx *operands;
{
  ...
  /* decr the counter by ALIGN */
  zoperands[0] = xoperands[2];
  zoperands[3] = plus_constant (xoperands[2], -align);
  output_load_address (zoperands);
#endif
  ...
  /* -align (xoperands[4]) is is in the range -1 to -4.  Load using adds */
  output_asm_insn ("adds %4,r0,%5", xoperands);

  xoperands[6] = operands[6];
  output_asm_insn ("adds %0,%2,%6", xoperands);
  output_asm_insn ("adds %5,%2,%2", xoperands);	/* Shrink counter by ALIGN */
/*  output_asm_insn ("subu %1,%5,%1", xoperands);	 Adjust SRC for counter */
  ...
  if (align == 1)
    {
      ...
      output_asm_insn ("st.b r31,%4(%6)", xoperands);
  ...
  if (align == 4)
    {
      ...
      output_asm_insn ("st.l r31,%4(%6)", xoperands);
      output_asm_insn ("bla %5,%2,.Lm%3", xoperands);
      output_asm_insn ("adds %5,%6,%6", xoperands);
    }
     ...
     rtx insn;
{
  ...
  if (GET_CODE (src) == SYMBOL_REF || GET_CODE (src) == CONST
	  ...
	  && !(LOGIC_INT (src) || (INTVAL (src) & 0xFFFF) == 0)))
    {
      rtx xoperands[2];
      xoperands[0] = dest;
      xoperands[1] = src;
  ...
  else
    {
      ...
      char *dtemplate;		/* Delay candidate template */
      ...
      for (i = 0; i < insn_n_operands[insn_code_number]; i++)
	{
	 ...
	 need to do an insn_extract.  */
      dtemplate = insn_template[insn_code_number];
      if (dtemplate == 0)
	dtemplate = (*insn_outfun[insn_code_number]) (recog_operand, delay_insn);
      output_asm_insn (dtemplate, recog_operand);
