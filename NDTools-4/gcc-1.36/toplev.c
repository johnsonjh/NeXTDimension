...
/* Called from final_scan_insn (final.c) to give a better error message when we
   have an insn whose operands don't match constraints, rather
   than just calling abort().  */

void
fatal_insn_not_constrained (insn)
     rtx insn;
{
  error ("The following insn had unmatched operands:", 0);
  debug_rtx (insn);
  abort ();
}
