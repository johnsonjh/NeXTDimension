...
#if defined(CROSS) && defined(I860)
struct compiler compilers[] =
{
  {".c",
   "/lib/cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{trigraphs} -undef \
  ...
  {".s",
   "/lib/cpp -+ %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} \
	...
	%{Wcomment*} %{Wtrigraphs} %{Wall} %C\
     ...
     %{!S:as860 %{R} %{j} %{J} %{h} %{d2} %a \
            %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o} %g.s\n }"},
#if 0
  {".s",
   "%{!S:as860 %{R} %{j} %{J} %{h} %{d2} %a \
...
#endif
  {".S",
   "/lib/cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{trigraphs} \
        -undef -D__GNUC__ -$ %p %P\
        %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\
	%{Wcomment*} %{Wtrigraphs} %{Wall} %C\
        %i %{!M*:%{!E:%{!pipe:%g.s}}}%{E:%{o*}}%{M*:%{o*}} |\n\
    %{!M*:%{!E:%{!S:as860 %{R} %{j} %{J} %{h} %{d2} %a \
                    %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o}\
		    %{!pipe:%g.s}\n }}}"},
  /* Mark end of table */
  {0, 0}
...
char *link_spec = "%{!c:%{!M*:%{!E:%{!S:ld860 %{o*} %l\
 %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\
 %{y*} %{!nostdlib:%S} \
 %{L*} %o %{!nostdlib:runtime860.a%s %{g:-lg} %L}\n }}}}";
 
#else	/* Use standard spec */
