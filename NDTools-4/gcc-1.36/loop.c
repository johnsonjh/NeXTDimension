...
#if defined(NeXT) && defined(HOIST_SUBREGS)
      if (GET_CODE (p) == INSN
	  && GET_CODE (PATTERN (p)) == SET
	  && ((GET_CODE (SET_DEST (PATTERN (p))) == REG
	      && ! may_not_move[REGNO (SET_DEST (PATTERN (p)))])
	    ||(GET_CODE (SET_DEST (PATTERN (p))) == SUBREG 
	      && ! may_not_move[REGNO(SUBREG_REG(SET_DEST(PATTERN (p))))]) ) )
#else
      if (GET_CODE (p) == INSN
	  && GET_CODE (PATTERN (p)) == SET
	  && GET_CODE (SET_DEST (PATTERN (p))) == REG
...
#if defined (NeXT) && defined(HOIST_SUBREGS)
	  rtx set_dest;
	  if ( GET_CODE (SET_DEST (PATTERN (p))) == REG )
	  {
		set_dest = SET_DEST (PATTERN (p));
	  }
	  else
	  {
		set_dest = SUBREG_REG(SET_DEST (PATTERN (p)));
	  }
#else
#define set_dest (SET_DEST (PATTERN (p)))
#endif
	  /* Don't try to optimize a register that was made
	     by loop-optimization for an inner loop.
	     We don't know its life-span, so we can't compute the benefit.  */
	  if (REGNO(set_dest) >= old_max_reg)
	    ;
	  /* If this register is used or set outside the loop,
	  ...
	  else if ((uid_luid[regno_last_uid[REGNO(set_dest)]] > INSN_LUID (end)
		    || uid_luid[regno_first_uid[REGNO(set_dest)]] < INSN_LUID (loop_start))
		   && (maybe_never
	    ...
	    ;
		   ...
		   && (n_times_set[REGNO(set_dest)] == 1
		       || (tem1
			   = consec_sets_invariant_p (set_dest,
		...
		m->set_src = SET_SRC (PATTERN (p));
	      m->force = 0;
	      m->consec = n_times_set[REGNO(set_dest)] - 1;
	      m->done = 0;
	      m->forces = 0;
	      m->partial = 0;
	      m->savemode = VOIDmode;
...
#if defined(NeXT) && defined(HOIST_SUBREGS)
      if ( GET_CODE (SET_DEST (PATTERN (m->insn))) == SUBREG )
	 dst_reg = SUBREG_REG(SET_DEST (PATTERN (m->insn)));
      else
	 dst_reg = SET_DEST (PATTERN (m->insn));
      
      if (!m->done
	  && (! m->cond
	      || (1 == invariant_p (m->set_src)
		  && (m->consec == 0
		      || 1 == consec_sets_invariant_p (dst_reg,
						       m->consec + 1,
						       m->insn))))
...
#ifdef NeXT
    case SUBREG:
      return invariant_p (SUBREG_REG (x));
#endif  /* NeXT */
  ...
  while (count > 0)
	...
	p = XEXP (temp, 0);
	  ...
	  && ( (GET_CODE (SET_DEST (PATTERN (p))) == REG
	    ...
	    || (GET_CODE (SET_DEST (PATTERN (p))) == SUBREG 
	      && REGNO(SUBREG_REG(SET_DEST(PATTERN (p)))) == regno)) )
#else
      if (code == INSN && GET_CODE (PATTERN (p)) == SET
	  && GET_CODE (SET_DEST (PATTERN (p))) == REG
	  && REGNO (SET_DEST (PATTERN (p))) == regno)
