...
#if defined(I860)
  text_siz = (text_siz + 0x1F) & (~0x1F);/* Keep 32 byte alignment (most restrictive)*/
...

...
#if defined(I860)
      data_siz += (16 - (data_siz % 16)) % 16; /* Pad data seg to preserve alignment */
      data_last_frag->fr_address = data_siz;   /* to quad-word boundries */
#endif
...
#if defined(I860)
  local_bss_counter=(local_bss_counter+0xF)&(~0xF); /* Pad BSS to preserve alignment */
#endif
  ...
  for (fragP = text_frag_root;  fragP;  fragP = fragP -> fr_next)
	...
	if(!lie->added) {
#if defined(I860)
	  fix_new(lie->frag,lie->word_goes_here - lie->frag->fr_literal,2,lie->add,lie->sub,lie->addnum,0,NO_RELOC);
#else
     ...
     int		this_segment_type; /* N_TYPE bits for segment. */
{
  ...
  for ( ;  fixP;  fixP = fixP -> fx_next)
	...
	case 0:
	   ...
	   */
	  if ( fixP->fx_addsy && 
	  	(fixP->fx_r_type==RELOC_HIGH || fixP->fx_r_type==RELOC_HIGHADJ) )
	  {
	    ++seg_reloc_count;
	  }
     ...
     relax_addressT	segment_address_in_file;
{
  ...
  for ( ;  fixP;  fixP = fixP -> fx_next)
	...
	ri . r_type		= fixP -> fx_r_type;
...
#if 0
		if((symbolP -> sy_type & N_TYPE) == N_DATA){
		    printf("r_address 0x%08x sy_value 0x%08x",
			   ri.r_address, symbolP->sy_value);
		    if(symbolP->sy_name)
			printf(" sy_name %s\n", symbolP->sy_name);
		    else
			printf("\n");
	   ...
	   */
	  if ( fixP->fx_r_type == RELOC_HIGH || fixP->fx_r_type == RELOC_HIGHADJ )
	  {
	  	ri.r_length		= nbytes_r_length [fixP->fx_size];
	  	ri.r_pcrel		= fixP->fx_pcrel;
		...
		/* Hide the low half of the addr in r_symbolnum.  More overloading...*/
