...
INT	kdbsigcode;	/* belongs in head.h */
...
setcor()
{
	...
	if (kernel && fcor != -1 && INKERNEL(filhdr.a_entry)) {
		...
		if (kernel == 0 && (stb.st_mode & S_IFREG))
			datmap.b1 = VM_MIN_KERNEL_ADDRESS;
		lookup("_Sysmap");
		sbr = cursym->n_value;
		lookup("_Syssize");
		slr = cursym->n_value;
	...
	if (read(fcor, (char *)&u, ctob(UPAGES))!=ctob(UPAGES) ||
	    !INUDOT(u.u_pcb.pcb_tss.t_esp0) || !INSTACK(u.u_pcb.pcb_tss.t_esp)) {
		datmap.e1 = MAXFILE;
		return;
	}
	signo = u.u_arg[0];
...
/*
	lseek(fcor, masterpcbb&~VM_MIN_KERNEL_ADDRESS, 0);
