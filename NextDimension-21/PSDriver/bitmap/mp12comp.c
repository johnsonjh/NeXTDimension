/*****************************************************************************
    ...
    mp12comp.c (used to be moverect.c)
...
#if INCLUDE_WF_HARDWARE
static const MLDPROC moveLineDispatch[2][6][4] = {
...
static const MLDPROC moveLineDispatchSW[2][6][4] = {
#else
static const MLDPROC moveLineDispatch[2][6][4] = {
#endif
 {{WCOPYConLine, RWCOPYBmULine, WCOPYPatLine, RWCOPYBmALine },
...
void SetUpSource(BitsOrPatInfo **psi, LineOperation *lo, int func, int lToR)
	/* psi indirected so we can zero it for real SCON */
{
    ...
    /* If the function is not standard copy, then we might have to
     * modify the dstPtr address because of our secret hardware.
     */
#if INCLUDE_WF_HARDWARE
    if (use_wf_hardware) {
	if (func != WCOPY && func != WF4)
	    if ((mpAddr <= lo->loc.dstPtr) &&
		(lo->loc.dstPtr < mpAddr + screenOffsets[0]))
		lo->loc.dstPtr += screenOffsets[func-1] >> 2;
	    else
		lo->loc.dstPtr += memoryOffsets[func-1] >> 2;
    }
#endif
    lo->proc = (MLDPROC) moveLineDispatch[lToR][func][lo->source.type];
}
...
#define CONSTSOURCE(n, val) \
    ...
    lO[(n)].proc = (MLDPROC) moveLineDispatch[lToR][WCOPY][SCON]; \
    sI[(n)] = NULL;

#if INCLUDE_WF_HARDWARE
#define BUFFERSOURCEHW(n, func) \
    ...
    if (func != WCOPY && func != WF4) \
	if ((mpAddr <= lO[(n)].loc.dstPtr) \
	&& (lO[(n)].loc.dstPtr < mpAddr + screenOffsets[0])) \
	    lO[(n)].loc.dstPtr += screenOffsets[func - 1] >> 2; \
	else \
	    lO[(n)].loc.dstPtr += memoryOffsets[func - 1] >> 2; \
    lO[(n)].proc = (MLDPROC) moveLineDispatch[lToR][func][SBMA];
#endif
...
#if INCLUDE_WF_HARDWARE
    if (!use_wf_hardware)
	bcopy(moveLineDispatchSW, moveLineDispatch, sizeof moveLineDispatch);
#endif
    moverectlen = INITIAL_MOVERECT_LEN;
    moverectbuff = (uint *)malloc(moverectlen*sizeof(uint));
#if INCLUDE_WF_HARDWARE
    if (use_wf_hardware)
	MP12SetBitmapExtent(moverectbuff, moverectbuff + moverectlen);
#endif
}
...
void MRMoveRect(BitsOrPatInfo *sI[3], BitsOrPatInfo *dI, int op, int d_ofs,
		int height, int argNumInts, int numLastPix, int lToR)
{
    ...
    if (numInts > moverectlen - MOVERECTSAFETY) {
	free(moverectbuff); /* Free old guy */
	moverectlen = numInts + MOVERECTSAFETY; /* Grow moverectbuff */
	moverectbuff = (uint *)malloc(moverectlen*sizeof(uint));
#if INCLUDE_WF_HARDWARE
	if (use_wf_hardware)
	    MP12SetBitmapExtent(moverectbuff, moverectbuff + moverectlen);
    ...
    MRMasks(&lO[0].loc.leftMask, &lO[0].loc.rightMask, d_ofs, &numInts,
	numLastPix);
	...
	case 1:
		...
		case FIRSTF1OP + 2:	/* D = S */
		    ...
		    {
			lO[0].source.data.bm.leftShift <<= MP12LOG2BD;
	...
	case 2:
		...
		case FIRSTF2OP + 0:	/* D = S0 + (D*(1-S1)) */
		    ...
		    {
			lO[1].source.data.bm.leftShift <<= MP12LOG2BD;
	...
	case 3:
		...
		case FIRSTF1OP + 0:	/* D = (1-D)*S */
		    sI[1] = dI;
#if INCLUDE_WF_HARDWARE
		    if (use_wf_hardware) { BUFFERSOURCEHW(2, WCOPY); } else
#endif
		    BUFFERSOURCE(2, WCOPY);
		    break;
		case FIRSTF2OP + 6:	/* D = D + S0*(1-S1) */
#if INCLUDE_WF_HARDWARE
		    if (use_wf_hardware) { BUFFERSOURCEHW(2, WF1); } else
#endif
		    BUFFERSOURCE(2, WF1);
		    break;
		default:
...
#if INCLUDE_WF_HARDWARE
		if (use_wf_hardware) { BUFFERSOURCEHW(3, WF1); } else
#endif
		{ BUFFERSOURCE(3, WF1); }
		while (height-- > 0)
		{
...
void MRPatternAdvance(LineSource *lS, BitsOrPatInfo *sI)
{
    if (lS->type == SPAT)
    {
