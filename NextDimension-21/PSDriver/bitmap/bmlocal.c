 ...
 * 23Aug90 Ted     Updated from ps version.
 */
...
static Bitmap *LBMNewFromData(LocalBMClass *class, Bounds *b, void *bits,
...
static void LBMFree(Bitmap *bm)
{
	...
	if(lbm->abits) vm_deallocate(task_self(), (vm_address_t)lbm->abits,
				     lbm->byteSize);
	break;
    }
    (*bmClass->_free)(bm);	/* super free */
}
