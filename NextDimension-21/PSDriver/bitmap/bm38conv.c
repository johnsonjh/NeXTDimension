...
#ifndef i860
/* HACK: WARNING, this code doesn't do mpto32 conversion, so it won't work
 * without removing the hacks in process.c and bm38.c
 */
...
void BM38Convert16to32(LocalBitmap *dbm, LocalBitmap *sbm, Bounds db,
    Bounds sb)
{
    unsigned int *dp, d, s;
    unsigned short *sp;
...
void BM38Convert8to32(LocalBitmap *dbm, LocalBitmap *sbm, Bounds dBounds,
    Bounds sBounds)
{
    unsigned int *dp;
    ...
    w = sBounds.maxx - sBounds.minx;
    h = sBounds.maxy - sBounds.miny;
    sp = (unsigned char *)sbm->bits;
    ap = (unsigned char *)sbm->abits;
    dp = dbm->bits;

    sinc = sbm->rowBytes;
    x = sBounds.minx - sbm->base.bounds.minx +
	  sinc*(sBounds.miny - sbm->base.bounds.miny);
    sp += x;
    if (ap) ap += x;
    sinc -= w;

    dinc = dbm->rowBytes>>2;
    dp += sBounds.minx - dbm->base.bounds.minx +
	  dinc*(dBounds.miny - dbm->base.bounds.miny);
    dinc -= w;
