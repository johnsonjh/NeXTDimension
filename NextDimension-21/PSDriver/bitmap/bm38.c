...
{
    *rowBytes = ((b->maxx - b->minx)*4 + 7)&~7;	/* double aligned */
			    ...
			    Bounds *sBounds, DevPoint phase)
{
    ...
    switch(sb->type) {
    case NX_OTHERBMTYPE:
	if (sb->isa != mp12) break; /* HACK: Allow mp bitmaps to go through */
    case NX_TWOBITGRAY:
			     ...
			     PatternData *data)
{
	...
	(markInfo->priv ? ALPHAVALUE(markInfo->priv) : OPAQUE);
    data->id = 0;
}
