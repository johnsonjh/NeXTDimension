...
public DevColor ConvertColorRGB (device, colorSpace, input, tfr, priv, gt, r)
  ...
  switch (colorSpace) {
    case DEVGRAY_COLOR_SPACE: {
      red = green = blue = gray =
        (integer)(input->value.gray * fp255 + fpHalf);
      break;
      }
    case DEVRGB_COLOR_SPACE: {
      red = (integer) (input->value.rgb.red * fp255 + fpHalf);
      green = (integer) (input->value.rgb.green * fp255 + fpHalf);
      blue = (integer) (input->value.rgb.blue * fp255 + fpHalf);
      gray = (integer)(
	colorWr * input->value.rgb.red + colorWg * input->value.rgb.green +
	colorWb * input->value.rgb.blue + fpHalf);
      break;
      }
    case DEVCMYK_COLOR_SPACE: {
      red = (integer) ((fpOne - input->value.cmyk.cyan) * fp255 + fpHalf);
      green = (integer) ((fpOne - input->value.cmyk.magenta) * fp255 + fpHalf);
      blue = (integer) ((fpOne - input->value.cmyk.yellow) * fp255 + fpHalf);
      gray = (integer) (input->value.cmyk.black * fp255 + fpHalf);
...
public DevColor ConvertColorCMYK (device, colorSpace, input, tfr, priv, gt, r)
  ...
  switch (colorSpace) {
    ...
    case DEVCMYK_COLOR_SPACE: {
      red = (integer) (input->value.cmyk.cyan * fp255 + fpHalf);
      green = (integer) (input->value.cmyk.magenta * fp255 + fpHalf);
      blue = (integer) (input->value.cmyk.yellow * fp255 + fpHalf);
      gray = (integer) (input->value.cmyk.black * fp255 + fpHalf);
      break;
      }
    }
