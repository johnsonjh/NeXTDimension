...
extern procedure ImIdent38();/* identity xform, no transfer func, 32->32 bit */
    ...
    ImageArgs *args;
{
	...
	case 16:	/* dest bitsPerPixel */
		...
		/* Special case 32-bit RGBA or RGB interleaved source
		 * identity transform, no transfer function
		 */
		if (source->nComponents == 3 && source->bitspersample == 8 &&
		    source->interleaved &&
		    (!im->transfer || !im->transfer->white)) {
		    Mtx *m = im->info.mtx;
		    if (m->b == 0.0 && m->c == 0.0 &&
    ...
    if (source->interleaved && im->unused && args->bitsPerPixel == 2)
        return;		/* Ted - Changed from PSRangeCheck() */
