...
static inline int PrebuiltPathSearch(char *fontName, int length)
{
    ...
    return FontPathSearch(fontName,fileName,filePath,sizeof(filePath));
}
...
static void readPrebuiltFile(NextPrebuiltFont *npf, char *path)
{
    char *charSet, *names;
    PrebuiltFile *file;
    int i;
    ...
    if(!(npf->stm = os_fopen(path, "r"))) return;
...
static inline boolean CheckPrebuiltMatrix(Mtx *m)
{
    ...
    if((d = m->d) < 0) d = -d;
    a = m->a;
    return (m->b==0. && m->c==0. && a<MAXIMUMSIZE && a>MINIMUMSIZE && a==d);
}	
