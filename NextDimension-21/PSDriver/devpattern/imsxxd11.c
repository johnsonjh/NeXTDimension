...
Paul Rovner: Fri Dec 29 11:09:15 1989
Jim Sandman: Wed Aug  2 12:17:33 1989
Steve Schiller: Mon Dec 11 13:45:55 1989
pgraff@next 5/30/90 fixed typo on line 328 (was sOrigX, fixed to sOrigY)
End Edit History.
*/
...
public procedure ImSXXD11(items, t, run, args)
  integer items;
  DevTrap *t;
  DevRun *run;
  ImageArgs *args; 
  ...
  register unsigned char *sLoc, *screenElt, *eScrnRow, *tfrGry;
  ...
  if (args->bitsPerPixel != 1 || args->gray.first != 0 || args->gray.n != 2
    || args->gray.delta != 1)
	  ...
	  if (sv.y > 0) {
		...
		switch (sourceType) {
		  ...
		  case s13I: {
		    ...
		    srcPix = (x & 7);
		    sData = *srcLoc;
		    data0 = (1 & (sData >> (7 - srcPix))) ? 255 : 0;
		    if (srcPix++ >= 8) {
		      srcLoc++;
		      sData = *srcLoc;
		      srcPix = 0;
		      }
		    data1 = (1 & (sData >> (7 - srcPix))) ? 255 : 0;
		    if (srcPix++ >= 8) {
		      srcLoc++;
		      sData = *srcLoc;
 		      srcPix = 0;
		      }
		    data2 = (1 & (sData >> (7 - srcPix))) ? 255 : 0;
		  ...
		  case s23I: {
		    ...
		    srcPix = (x & 7);
		    sData = *srcLoc;
		    data0 = twoBitSample[3 & (sData >> (6 - srcPix))];
		    srcPix += 2;
		    if (srcPix >= 8) {
		      srcLoc++;
		      sData = *srcLoc;
		    ...
		    data1 = twoBitSample[3 & (sData >> (6 - srcPix))];
		    srcPix += 2;
		    if (srcPix >= 8) {
		      srcLoc++;
		      sData = *srcLoc;
		    ...
		    data2 = twoBitSample[3 & (sData >> (6 - srcPix))];
		    sData = FRound(Wr * data0 + Wg * data1 + Wb * data2);
		    if (sData > 255) sData = 255;
		    break;
		    }
		  ...
		  case s13N: {
		    ...
		    srcPix = 7 - (x & 7);
		    data0 = 
		      ((*(sample0 + loc) >> srcPix) & 1) ? 255 : 0;
		    data1 =
		      ((*(sample1 + loc) >> srcPix) & 1) ? 255 : 0;
		    data2 =
		      ((*(sample2 + loc) >> srcPix) & 1) ? 255 : 0;
		    sData = FRound(Wr * data0 + Wg * data1 + Wb * data2);
		    if (sData > 255) sData = 255;
		    break;
		    }
		  ...
		  case s14N: {
		    ...
		    srcPix = 7 - (x & 7);
		    data0 = 
		      ((*(sample0 + loc) >> srcPix) & 1) ? 0 : 255;
		    data1 =
		      ((*(sample1 + loc) >> srcPix) & 1) ? 0 : 255;
		    data2 =
		      ((*(sample2 + loc) >> srcPix) & 1) ? 0 : 255;
		    data3 =
		      ((*(sample3 + loc) >> srcPix) & 1) ? 255 : 0;
		    sData = FRound(
		      Wr * data0 + Wg * data1 + Wb * data2) - data3;
		    if (sData < 0) sData = 0;
		    if (sData > 255) sData = 255;
		...
		if (tfrGry != NULL) sData = tfrGry[sData];
	        }
